...
var allLoader = require('./loader.all');

var allPlugin = require('./plugin.all'); // 是否使用 ts
...
module.exports = function (webpackEnv, overwrite) {
  ...
  var whichEnv = {
    ...
    isNotEnvDevelopment: isNotEnvDevelopment,
    isEnvProduction: isEnvProduction
  }; // sourceMap 控制所有 sourceMap是否生成
  // 若命令行未指定 sourceMap, 则生产环境不生成sourceMap，其它环境都生成

  var sourceMap = overwrite.sourceMap === undefined ? !isEnvProduction : Boolean(overwrite.sourceMap); // webpack devtool 字段指定 sourceMap 类型 inline-source-map,source-map,eval .etc

  var devtool = sourceMap && (isNotEnvDevelopment ? 'source-map' : 'cheap-module-source-map');
  var publicPath = isEnvDevelopment ? '/' : paths.publicUrl; // Rules

  var rules = allLoader(whichEnv, overwrite); // Plugins

  var plugins = allPlugin(whichEnv, overwrite); // 重写 HTML 配置

  if (overwrite.webpack && Array.isArray(overwrite.webpack.plugins) && overwrite.webpack.plugins.filter(function (i) {
    return i instanceof HtmlWebpackPlugin;
  }).length) {
    plugins = plugins.filter(function (i) {
      return !(i instanceof HtmlWebpackPlugin);
    });
  }
  ...
  return merge({
     ...
     */
    entry: [// 热更新
    isEnvDevelopment && require.resolve('react-dev-utils/webpackHotDevClient')].concat((0, _toConsumableArray2.default)(fs.existsSync(paths.appPolyfill) ? [paths.appPolyfill] : [require.resolve('react-app-polyfill/ie11'), require.resolve('./polyfills.js')]), [paths.appIndexJs]).filter(Boolean),
    // 描述 chunk 的存放信息
    output: {
      // chunk输出目录
       ...
       */
      filename: isNotEnvDevelopment ? 'js/[name].[chunkhash:8].js' : 'js/bundle.js',
      // 非入口 chunk (code split)名称
      chunkFilename: isNotEnvDevelopment ? 'js/[name].[chunkhash:8].bundle.js' : 'js/[name].bundle.js',
      publicPath: publicPath,
      // sourceMap 地址指向为原始磁盘路径
      devtoolModuleFilenameTemplate: isNotEnvDevelopment ? function (info) {
    ...
    resolve: {
      ...
      // 指定 webpack 查找模块的目录
      modules: ['node_modules'].concat( // `env.js` 确保 process.env.NODE_PATH 存在
      ...
      plugins: [// 为 webpack 提供即插即用特性
      PnpWebpackPlugin, // 防止用户引用 src 或 node_modules 外的文件
      new ModuleScopePlugin(paths.appSrc, [paths.appPackageJson])]
    },
    // 解析 loader 路径
    resolveLoader: {
      plugins: [// 指定 webpack 从当前包加载 loader
