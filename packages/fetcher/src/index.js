...
import {
    isObject, isString, isNull,
    isPlainObject, isBoolean, isFunction,
} from '@ruhnn/util/type';
...
export function oriFetch(url, option = {}) {
    const opt = {credentials: 'same-origin', ...(isObject(option) ? option : {})};
    ...
    // 请求头带上Token
    if (window.token) {
        opt.headers.Token = 'token' in opt.headers || 'Token' in opt.headers
            ? opt.headers.Token
            : window.token;
    }
...
const loginFulfill = data => {
    ...
    const delay = () => new Promise(
        (resolve, reject) => setTimeout(reject, 99999, data),
    );
    const errorCallback = () => {
        try {
            if (isLCAPP) {
...
const errorFulfill = error => data => {
    ...
    // 请求错误或响应成功 则不提示
    if (!isPlainObject(data) || data.success) return data;

    // 服务端错误处理
    let serverError = data.errorMessage || data.errorMsg;
    if (serverError === '系统错误') serverError = '操作失败';
    if (data.errorCode === 'B10') serverError = '没有权限';

    // 错误信息不存在 且 message===false 则不提示
    if (msg === false && !serverError) return data;
...
const validateFulfill = dataType => data => {
    const {canBeNull = true, message = '数据错误', type} = dataType;
    const dataFormatError = new DataFormatError(message);
 ...
 * @return {Function}
 */
const errorReject = ({type, always}) => e => {
    ...
    if (always) return {result: type ? typeDefault[type] : undefined};
    return e;
};
...
function fetcher(url, option = {}) {
    // 自定义参数处理
    let {dataType, error} = option;
    // dataType
    if (isString(dataType)) dataType = {type: dataType};
    if (!isObject(dataType) || !(dataType.type in typeDefault)) dataType = null;
    const toValid = isObject(dataType);
    // error
    if (isString(error) || isBoolean(error) || isFunction(error)) error = {message: error};
    if (!isObject(error)) error = null;
    const toMessage = isObject(error);
    // 过滤自定义参数
    const opt = {...option, dataType: undefined, error: undefined};

    const func = () => oriFetch(url, opt)
        .then(res => res.json())
        // 登录验证
        .then(loginFulfill)
        // 服务端错误提示
        .then(toMessage ? errorFulfill(error) : undefined)
        // 数据格式验证
        .then(toValid ? validateFulfill(dataType) : undefined)
        // 错误处理
