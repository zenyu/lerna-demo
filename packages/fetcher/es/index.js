...
if (mobile) {
  ...
  require('antd-mobile/lib/toast/style/css'); // eslint-disable-next-line
  ...
  // eslint-disable-next-line
  require('antd/lib/message/style/css'); // eslint-disable-next-line
...
export function oriFetch(url, option = {}) {
  ...
  let o = {};
  let contentType = ''; // 针对不同请求方式 添加请求头 或 参数处理
...
export default function fetcher(url, option = {}, format) {
  ...
  return abort(() => oriFetch(url, option).then(res => res.json()).then(data => {
    // 传入 format 参数 会进行数据格式验证
    // 字符串类型且在
    ...
    const dataFormatError = new DataFormatError(format.message); // 字段验证 success result

    if (toString.call(data) !== '[object Object]' || !('success' in data) || !('result' in data)) {
      throw dataFormatError;
    } // data.success 为 false 交给业务端处理


    if (!data.success) return data; // data.result 数据类型验证

    const dataType = toString.call(data.result).toLowerCase(); // data.result 类型不正确
    ...
    if (data.errorCode === 'LOGIN_ERROR' || data.errorCode === 'C10') {
      return message.error('登录信息过期，请重新登录', 1.5, () => errorCallback(data));
    }
    ...
    console.log('fetcher error >> ', e); // 合并错误提示
